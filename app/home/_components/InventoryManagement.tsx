...
const InventoryManagement = ({ orgId }: Props) => {
    ...
    const fetchData = async () => {
        const [{ data: inv }, { data: orgProfile }] = await Promise.all([
            supabase.from('organization_inventory').select('food, water, clothes').eq('org_id', orgId).single(),
            supabase.from('organization_profiles').select('urgent_need').eq('profile_id', orgId).single(),
        ])
    ...
    const isUrgent = (type: ItemType) => urgentNeed?.toLowerCase() === type.toLowerCase()
        ...
        <div className="flex flex-col h-full gap-3">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {ITEMS.map(item => (
                    <div
                        key={item.type}
                        className={`flex flex-col items-center justify-center p-3 sm:p-4 rounded-2xl border-2 ${item.bg} ${item.border} transition-all duration-300`}
                    >
                        {loading ? (
                            <div className="w-10 h-8 bg-white/60 rounded-lg animate-pulse mb-2" />
                        ) : (
                            <span className={`text-2xl sm:text-3xl font-black ${item.color} leading-none`}>
                                {inventory[item.key]}
                            </span>
                        )}
                        <div className="flex items-center gap-1 mt-1.5 sm:mt-2">
                            <span className="text-sm sm:text-base">{item.icon}</span>
                            <span className={`text-[9px] sm:text-xs font-black uppercase tracking-widest ${item.color}`}>
                                {item.type}
                            </span>
                        </div>
                        <span className="text-[9px] sm:text-[10px] text-slate-400 font-medium mt-0.5 sm:mt-1">units</span>
                    </div>
                ))}
            </div>
                ...
                {ITEMS.map(item => {
                    ...
                    return (
                        <button
                            ...
                            className={`relative flex flex-col items-center justify-center gap-1 py-2.5 sm:py-3 px-1 sm:px-2 rounded-2xl border-2 font-black text-[9px] sm:text-xs uppercase tracking-widest transition-all duration-200 disabled:opacity-50
                                ${active
                        ...
                        >
                            {active && (
                                <span className="absolute -top-2 -right-2 w-4 h-4 sm:w-5 sm:h-5 bg-red-500 rounded-full flex items-center justify-center shadow-md">
                            ...
                            <span>{item.type}</span>
                            {active && (
                                <span className="text-[8px] sm:text-[9px] font-black opacity-70 text-center leading-tight">URGENT ✕ tap to clear</span>
                            )}
                        </button>
                    )
                })}
