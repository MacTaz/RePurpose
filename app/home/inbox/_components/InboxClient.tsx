...
const InboxClient = ({ role, userId, userDisplayName }: InboxClientProps) => {
    ...
    const fetchConversations = async () => {
        ...
        // Step 3: fetch all donation details in parallel, keyed by id
        ...
        // Step 4: build groupMap synchronously — no async, no races
    ...
    // ── Global inbox realtime: update thread order + unread bubble ────────────
    useEffect(() => {
        const inboxChannel = supabase
            .channel(`inbox_messages:${userId}`)
            .on('postgres_changes', {
                ...
                setOrgGroups(prev => {
                    const groupIdx = prev.findIndex(g =>
                        g.threads.some(t => t.conversationId === convoId)
                    )
                    if (groupIdx === -1) return prev

                    const newTime = new Date(m.created_at).toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit' })

                    const groups = prev.map((g, gi) => {
                        ...
                        return { ...g, threads: newThreads }
                    })

                    groups.sort((a, b) => {
                        const aTime = a.threads[0]?.time || ''
                        const bTime = b.threads[0]?.time || ''
